...
class GP8403():
    ...
    def set_dac_out_voltage(self, data, channel):
        ...
        try:
            data = float(data)
            self.voltage_range
            if self.voltage_range >= data >= 0: 
                self.dataTransmission = int((float(data) / self.voltage_range) * 4095)
                ...
                return {'error':f'input voltage not in range of 0 - {self.voltage_range}'}
        except Exception as e:
            return {'error':f'invalid data input, Error = {e}'}
