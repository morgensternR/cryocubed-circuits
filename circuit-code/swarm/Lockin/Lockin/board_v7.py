...
def doCommand(cmd, responder=print):
    ...
    if len(cmd):  # respond to command
        ...
        if (cmd[0]=='Q'):
            ...
            responder({'LAST?' : f'{last}'})
        elif (cmd=="BIAS?"):
            response = p9.value()
            responder({'BIAS?' : f'{response}'})
        elif (cmd=="BIAS"):
            if len(args)>1:
                print('args', args);
                channel = int(args[1])
...
if (__name__ == 'board_v7') or (__name__=='__main__'):
    ...
    def readN_reset(N, skip=4, reset=False):
        ...
        while True:
            ...
            #print( np.sum((np.max(readings, axis=0)-np.min(readings, axis=0))) )
            if     np.sum((np.max(readings, axis=0)-np.min(readings, axis=0)) > ALLOWED_MIN_MAX_DIFF) == 0:
                break
            #else:
